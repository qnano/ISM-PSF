...
xvec{1} = FresnelP.*CosTheta; %xincident to x output
xvec{2} = zeros(size(CosTheta));                           %xincident to y output
xvec{3} = FresnelS.*SinTheta; %xincident to z output

yvec{1} = zeros(size(CosTheta));  
yvec{2} = FresnelP.*CosTheta;
yvec{3} = FresnelS.*SinTheta;
...
for itel = 1:3
  for jtel = 1:2
      ...
      PupilMatrix{itel,jtel} = Amplitude.*PhaseFactor;
  end
end
