...
xvec{1} = FresnelS.*CosTheta.*SinPhi+FresnelP.*CosPhi; %xincident to x output
xvec{2} = zeros(size(CosTheta));                           %xincident to y output
xvec{3} = FresnelS.*SinTheta.*SinPhi; %xincident to z output

yvec{1} = zeros(size(CosTheta));  
yvec{2} = FresnelS.*CosTheta.*CosPhi+FresnelP.*SinPhi;
yvec{3} = FresnelS.*SinTheta.*CosPhi;
